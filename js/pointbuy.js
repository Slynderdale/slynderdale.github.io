...
var AttributeList = ['AttributeStr','AttributeDex','AttributeCon','AttributeInt','AttributeWis','AttributeCha'];
...
function getTotals() {
  ...
  $('span#MaxAttribute').html(AttributeMax);
...
function getCurrentCost() {
  ...
  return parseInt(PointcostStr) + parseInt(PointcostDex) + parseInt(PointcostCon) + parseInt(PointcostInt) + parseInt(PointcostWis) + parseInt(PointcostCha);
}
...
function updatePoints(pointsAvail, pointsMax) {
  var pointsAvail = parseInt(pointsAvail)
  var pointsMax = parseInt(pointsMax)
  if (pointsAvail >= 1 && pointsAvail <= 99) {
    $('input#AvailablePoints').val(pointsAvail);
  }
  if (pointsMax >= 9 && pointsMax <= 18) {
    $('input#AttributeMax').val(pointsMax);
  }
...
function getRace(hsubrace, hattr1, hattr2) {
  ...
  $.getJSON( 'json/races_20200220.json', function( data ) {
    var html = [];
    var RaceChoice = $('select#Race').val();
    ...
    if (RaceChoice === 'HalfElf' ) {
      $('td#AbilityOption1').css('display', 'table-cell');
      $('td#AbilityMod1').css('display', 'table-cell');
      $('td#AbilityOption2').css('display', 'table-cell');
      $('td#AbilityMod2').css('display', 'table-cell');
    }
    else if (RaceChoice === 'Changeling' || RaceChoice === 'SimicHybrid' || RaceChoice === 'Warforged' ) {
      $('td#AbilityOption1').css('display', 'table-cell');
      $('td#AbilityMod1').css('display', 'table-cell');
      $('td#AbilityOption2').css('display', 'none');
      $('td#AbilityMod2').css('display', 'none');
      ...
      $('td#AbilityOption1, td#AbilityOption2').css('display', 'none');
      $('td#AbilityMod1, td#AbilityMod2').css('display', 'none');
    }
    ...
    raceReset();
      ...
      $('select').change(function() {
        ...
        $('select#ability1, select#ability2').not(this).children('option[value=selected], option[value=RacialCha]').attr('disabled', true);
      });
      $('select#ability1, select#ability2').children('option[value=selected], option[value=RacialCha]').attr('disabled', true);
      $('select#ability1 option').eq(hattr1).prop('selected', true);
      ...
      $('td#SelectSubrace').html('<select id="subRace" onchange="raceHalfElf()" style="width: 90%;"><option value="selectSub" selected data-srchoice="disabled">Disabled</option><option value="Half-Elf Variant" data-srchoice="enable">Enabled</option></select>');
    }
      ...
      $('select').change(function() {
        $('select#ability1, select#ability2').not(this).children('option[value=' + this.value + ']').attr('disabled', true).siblings().removeAttr('disabled');
        $('select#ability1, select#ability2').not(this).children('option[value=selected], option[value=RacialCon]').attr('disabled', true);
      });
      $('select#ability1, select#ability2').children('option[value=selected], option[value=RacialCon]').attr('disabled', true);
      $('select#ability1 option').eq(hattr1).prop('selected', true);
      $('select#ability1').change();
    }
...
function jsonRace() {
  ...
  $.getJSON( 'json/races_20200220.json', function( data ) {
    ...
    if ( race == 'Changeling' || race == 'HalfElf' ) {
...
function jsonSubrace() {
  $.getJSON( 'json/races_20200220.json', function( data ) {
    ...
    if (subrace === 'Human Variant' || race === 'HalfElf' ) {
      $('select#ability1').change();
      $('select#ability2').change();
    }
    getTotals();
...
function raceHuman() {
  ...
  jsonRace();
    ...
    $('td#AbilityOption1, td#AbilityOption2').css('display', 'table-cell');
    $('td#AbilityMod1, td#AbilityMod2').css('display', 'table-cell');
...
function increment(e) {
  ...
  if (abilityScore == abilityMax) {
    ...
    if (AttributeList.includes(abilityType)) {
      var AvailablePoints = parseInt($('input#AvailablePoints').val());
      var currentCost = getCurrentCost();
      var atrrivalue = parseInt($('input#' + [abilityType]).val())
      var attriCost = Points[atrrivalue] - Points[atrrivalue-1];
      var totalCost = attriCost + currentCost;
